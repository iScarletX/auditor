...
- 每条priority_action必须先填写problem_statement字段：用一到两句话客观描述“当前是什么样子、哪里不对”——这是现象本身，不含任何“应该怎么改”的建议性措词。与action_summary必须明确区分：problem_statement只说“现在是这样——为什么不行”，action_summary才说“应该改成这样”。不得把action_summary的内容换个说法搬进去充数，两个字段必须信息不重合。
...
只输出JSON：
{
  ...
  "prescription": {
    ...
    "priority_actions": [
      {
        "priority": 1,
        "problem_statement": "当前是什么样子、哪里不对（只描述现象，不包含改法）",
